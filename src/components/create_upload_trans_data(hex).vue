<template>
  ...
  <div>
    ...
    <div v-if="data_hex_flag" style="word-break:break-all">
      data:     {{this.data_hex}}
    </div>
  </div>
...
export default {
  name: "create_upload_trans_data(hex)",
  created() {
    this.test();
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          const path = 'http://127.0.0.1:5000/create_trasn_data';
          axios.post(path,(this.dynamicValidateForm)).then((res) =>{
            // console.log(res.data)
            this.data_hex=res.data;
            if(this.data_hex!=null){
              this.data_hex_flag=true
            }
